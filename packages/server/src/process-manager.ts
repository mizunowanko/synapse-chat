...
export class ProcessManager extends EventEmitter implements ProcessManagerLike {
  ...
  dispatchSortie(
    ...
    // Dispatch runs are transient. Callers that want a persistent log can
    // supply SYNAPSE_DISPATCH_LOG_DIR in extraEnv; otherwise dispatches run
    // without on-disk log tailing.
    const logDir =
      extraEnv?.SYNAPSE_DISPATCH_LOG_DIR ??
      process.env.SYNAPSE_DISPATCH_LOG_DIR;
    const logFilePath = logDir
      ? join(logDir, `dispatch-log-${id}.jsonl`)
      : undefined;
    this.setupProcess(id, proc, logFilePath);
    return proc;
  }
