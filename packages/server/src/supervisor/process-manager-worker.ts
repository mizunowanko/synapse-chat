 ...
 *
 * This process isolation ensures that CLI stdout/stderr parsing errors cannot
 * crash the WS/API server.
 */
...
process.on("message", (msg: ProcessManagerWorkerMessage) => {
  switch (msg.type) {
    ...
    case "request-state-dump":
      sendIpc({
        type: "state-dump",
        processes: Array.from(runningProcesses.entries()).map(([id, pid]) => ({
          id,
          pid,
        })),
      });
