 ...
 *
 * Used by the WS/API server process when running under the Supervisor.
 */
...
export class IpcProcessManager
  extends EventEmitter
  implements ProcessManagerLike
{
  ...
  private handleWorkerEvent(event: IpcEvent): void {
    switch (event.type) {
      ...
      case "state-dump":
        ...
        console.log(
          `[ipc-pm] State dump received: ${event.processes.length} running processes`,
        );
        break;
