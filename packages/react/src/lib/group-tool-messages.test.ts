...
describe("groupToolMessages", () => {
  ...
  it("keeps a single tool_use + tool_result inline", () => {
    const input = [msg("tool_use", { tool: "Read" }), msg("tool_result")];
    const result = groupToolMessages(input);
  ...
  it("includes tool_result in groups", () => {
    ...
    expect(group.messages.filter((m) => m.type === "tool_result")).toHaveLength(
      2,
    );
  });
  ...
  it("does not include assistant text inside tool_use groups", () => {
    const input = [
      msg("tool_use", { tool: "Bash" }),
      msg("tool_result"),
      msg("assistant"),
  ...
  it("does not group non-tool messages", () => {
    const input = [msg("assistant"), msg("user"), msg("system")];
    const result = groupToolMessages(input);
    expect(result).toHaveLength(3);
    expect(result.every((r) => !isToolGroup(r))).toBe(true);
  });
